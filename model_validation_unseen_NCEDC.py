# -*- coding: utf-8 -*-
"""
Created on Thu Sep 24 01:23:05 2020
...
file1 = 'NCEDC_new.hdf5'
dataPath = 'H:/MLData/groundMotion/Parkfield/new2'
...
for n in range(len(test_Y)):
    gap1.append(test_Y[n]-pred_y1[n]+1)
mean1 = np.mean(gap1)
sd1 = np.std(gap1)
print(mean1, sd1)
...
plt.show()
plt.savefig(os.path.join(figSavePath, 'freshTest_NCEDC.png'), dpi=60)
...
plt.grid()
plt.legend()
plt.savefig(os.path.join(figSavePath, 'freshTestDis_NCEDC.png'), dpi=60)
...
ax3.set_ylim([-2, 2])
plt.tight_layout()
plt.savefig(os.path.join(figSavePath, 'freshTestRes_NCEDC.png'), dpi=200)
plt.savefig(os.path.join(figSavePath, 'freshTestRes_NCEDC.eps'), dpi=200)



np.savetxt(os.path.join(figSavePath, 'fresh_NCEDC.txt'), gap1)
